...
// The `mut_from_ref` lint fires on every allocation method because bump
// allocators return `&mut T` from `&self`.  This is sound: `Bump` uses
// interior mutability (`Cell`) and each allocation returns a pointer to a
// unique, non-overlapping region of arena memory.
...
unsafe impl<const MIN_ALIGN: usize> alloc::Alloc for &Bump<MIN_ALIGN> {
    #[inline(always)]
