...
fn alloc_try_with_large_array_err() {
    ...
    assert!(
        b.alloc_try_with(|| Result::<[u8; 10_000_000], _>::Err(()))
            .is_err()
    );
}
...
fn alloc_try_with_large_struct_err() {
    ...
    assert!(
        b.alloc_try_with(|| Result::<LargeStruct, _>::Err(()))
            .is_err()
    );
}
...
fn alloc_try_with_large_tuple_err() {
    ...
    assert!(
        b.alloc_try_with(|| { Result::<(u32, LargeStruct), _>::Err(()) })
            .is_err()
    );
}
...
fn alloc_try_with_large_enum_err() {
    ...
    assert!(
        b.alloc_try_with(|| Result::<LargeEnum, _>::Err(()))
            .is_err()
    );
}
...
fn alloc_slice_try_fill_with_large_length() {
    ...
    assert!(
        b.alloc_slice_try_fill_with(10_000_000, |_| Err::<u8, _>(()))
            .is_err()
    );
}
...
fn alloc_slice_try_fill_iter_large_length() {
    ...
    let elems = std::iter::repeat_n(Err::<u8, _>(()), 10_000_000).collect::<Vec<_>>();
